...
def get_reports_by_reported(reported: str):
    ...
    """
    report_docs = db.collection('reports').where('reported', '==', reported).get()
    return [doc.to_dict() for doc in report_docs]

def get_reports_for_review():
    """
    Get all reports that are not yet approved.
    """
    report_docs = db.collection('reports').where('approved', '==', False).get()
    return [doc.to_dict() for doc in report_docs]

def update_report_status(report_id: str, approved: bool):
    """
